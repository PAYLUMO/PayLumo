 ...
 * `analyzePayslip` → `StoredAnalysis`. Débloquée seulement après paiement
 * vérifié (voir `app.ts`).
 */
...
/** Erreur « le bulletin n'est pas exploitable » → remboursement. */
export class UnreadableError extends Error {}
/** Erreur transitoire (réessai possible sans repayer). */
...
function buildStored(payslip: Payslip, reader: 'ai' | 'local', fileName: string): StoredAnalysis {
 ...
 */
export async function runAnalysis(pdfBase64: string, fileName: string): Promise<StoredAnalysis> {
  ...
  try {
    ...
    const stored = buildStored(payslipFromRaw(raw), 'ai', fileName);
    if (!stored.result.summary.canAnalyze) {
      // l'IA a répondu mais l'extraction est trop pauvre pour analyser
      return await tryLocalOrFail(pdfBytes, fileName);
    }
    return stored;
  } catch (err) {
    ...
    if (err instanceof ClaudeUnusableError) {
      ...
      try {
        return await requireAnalyzable(await readLocal(pdfBytes), fileName);
      } catch {
...
async function tryLocalOrFail(pdfBytes: Uint8Array, fileName: string): Promise<StoredAnalysis> {
  try {
    return await requireAnalyzable(await readLocal(pdfBytes), fileName);
  } catch (e) {
...
async function requireAnalyzable(payslip: Payslip, fileName: string): Promise<StoredAnalysis> {
  const stored = buildStored(payslip, 'local', fileName);
  if (!stored.result.summary.canAnalyze) {
